...
Rotated_Factors[abs(Rotated_Factors) < 0.5]<-NA
```
...
<br> 1. We need at least 6 components

<br> 2. The highest weights associated with the first principal components are 0.93, 0.86, 0.85.

<br> 3. Component 1: North American bonds, Component 2: Currencies, Component 3: European stock market indexes, Component 4: North American market indexes, Component 5: European bonds, Component 6: Oil, Component 7: Metals, Component 8: Agricultural commodities, Component 9: Precious metals, Component 10: Asian stock market indexes

<br> 4. Principal components and scree plot:
a) Pre-crisis years (bull markets)
```{r echo=TRUE, tidy=TRUE}
before_crisis_data = futures_data[ as.Date(rownames(futures_data)) < "2007-10-01", ]
before_crisis_components <- PCA(before_crisis_data, graph=FALSE)
before_crisis_table <- before_crisis_components$eig
colnames(before_crisis_table) <- c("Eigenvalue","Pct of explained variance", "Cumulative pct of explained variance")
eigen_before_crisis <- before_crisis_table[,"Eigenvalue"]
plot(eigen_before_crisis, col="red")
lines(rep(1,length(eigen_before_crisis)),col="green")
```

b) Financial-crisis years (bear markets)
```{r echo=TRUE, tidy=TRUE}
crisis_data = futures_data[ as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01" , ]
crisis_components <- PCA(crisis_data, graph=FALSE)
crisis_table <- crisis_components$eig
colnames(crisis_table) <- c("Eigenvalue","Pct of explained variance", "Cumulative pct of explained variance")
eigen_crisis <- crisis_table[,"Eigenvalue"]
plot(eigen_crisis, col="red")
lines(rep(1,length(eigen_crisis)),col="green")
```

<br> 5. Differences about the futures returns during  bull and bear markets
- Pre-crisis years (bull markets)
```{r echo=TRUE, tidy=TRUE}
before_corused = cor(before_crisis_data[,apply(before_crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results<-principal(before_corused, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors<-round(Rotated_Results$loadings,2)
Rotated_Factors<-as.data.frame(unclass(Rotated_Factors))
colnames(Rotated_Factors)<-paste("comp",1:ncol(Rotated_Factors),sep=" ")
...
show_data_before <- Rotated_Factors 
iprint.df(show_data_before, scale=TRUE)
```

- Financial-crisis years (bear markets)
```{r echo=TRUE, tidy=TRUE}
crisis_corused = cor(crisis_data[,apply(crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results<-principal(crisis_corused, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors<-round(Rotated_Results$loadings,2)
Rotated_Factors<-as.data.frame(unclass(Rotated_Factors))
colnames(Rotated_Factors)<-paste("comp",1:ncol(Rotated_Factors),sep=" ")
...
Rotated_Factors <- Rotated_Factors[sorted_rows,]
Rotated_Factors[abs(Rotated_Factors) < 0.3]<-NA
show_data_crisis <- Rotated_Factors 
iprint.df(show_data_crisis, scale=TRUE)
```

Components weigths are quite stable in both periods, even though returns look more correlated during crisis. In general commodities tend to be correlated in the same way before and during the crisis. 
...
1. It calculates moving averages using function "ma" using different windows defined by the vector "MAfreq". This vector defines both the lenght of the window and the number of different values that will be considered. Averages are scaled to obtain weights with total sum of 1 every day. After that, weights are multiplied with futures prices to define how much to invest every future day. Finally, the cumulative result is calculated as sum cross-future for every day.

2. Using a WMA instead of a normal MA, we obtain the following performances:
```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq <-seq(10,250,by=20)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + apply(futures_data,2, function(r) WMA(r,MAfreq[iter],wts=1:MAfreq[iter]))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used[is.na(signal_used)] <- 0
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
...
strategy_by_future <- scrub(shift(signal_used,2)*futures_data) # signal 2 days ago
# finally, this is our futures trend following strategy
trading_strategy2 = apply(strategy_by_future,1,sum)
