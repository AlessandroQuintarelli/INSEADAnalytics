...
*1. The code above calculates the moving averages using function "ma" with different windows defined by the vector "MAfreq". This vector is used both to define the lenght to be used and how many different values will be used. Averages are scaled to obtain weights with total sum of 1 each day. Then weights are multiplied by futures prices to define how much to invest on each future each day. Finally, cumulative result is calculated as sum cross-future for each day.

*2. Trying to use a WMA instead of a normal MA, we can obtain following performances:
```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq <-seq(10,250,by=20)
